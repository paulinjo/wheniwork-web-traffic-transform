...
import wheniwork.util.files
from wheniwork.util.files import read_file
from wheniwork.util.visits import PathVisit


class TestReadFile(unittest.TestCase):
    # Stub the file download
    wheniwork.util.files.urlopen = lambda x: MagicMock()
    header = ['a', 'b', 'c', 'd', 'e']

    def test_read_empty_file(self):
        wheniwork.util.files.reader = lambda x: [self.header]
    ...
    def test_read_file_one_row(self):
        row = ['1', '7', '/', 'Mozilla/5.0 (X11; Fedora; Linux x86_64; rv:54.0) Gecko/20100101 Firefox/54.0	',
               '378']
        wheniwork.util.files.reader = lambda x: [self.header, row]
    ...
    def test_read_file_two_rows_same_user(self):
        ...
        row2 = ['1', '7', '/', 'Mozilla/5.0 (X11; Fedora; Linux x86_64; rv:54.0) Gecko/20100101 Firefox/54.0	',
                '378']
        wheniwork.util.files.reader = lambda x: [self.header, row1, row2]
...
class TestWriteFile(unittest.TestCase):
    ...
    # wheniwork.util.files. = lambda x: MagicMock()
    ...
    def test_write_no_data(self, mock_open_file):
        wheniwork.util.files.write_file([], {}, 'path')
    ...
    def test_write_one_user(self, mock_open_file):
        ...
        wheniwork.util.files.write_file(paths, visits, 'path')
